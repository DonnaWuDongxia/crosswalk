 ...
 *     var helper = jsStub.create(exports, extension, false);
 *
 *     jsStub.makeEventTarget(exports);
 *     helper.addEvent("updatePrefix");
 *     helper.addEvent("click");
 *
 *     jsStub.defineProperty(exports, "prefix", true);
 *
 *     exports.echo = function(msg) {
 *       return helper.invokeNative("echo", [msg], true);
 *     }
 *
 *     exports.getPrefix = function(callback) {
 *        helper.invokeNative("getPrefix", [callback]);
 *     }
 *
...
jsStub.getHelper = function(base, channel) {
  if (!(base.__stubHelper instanceof jsStub)) {
    base.__stubHelper = new jsStub(base, channel);
  }
...
jsStub.makeEventTarget = function(base) {
  ...
  function dispatchEvent(type, event) {
    if (!(type instanceof String) || !(("on" + type) in base))
